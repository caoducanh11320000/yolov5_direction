...
cv::Point get_point(cv::Mat& img, cv::Point point){
  float scale_x = (float)img.cols/kInputW;
  float scale_y = (float)img.rows/kInputH;
  int new_x = scale_x*point.x;
  int new_y = scale_y*point.y;
...
void draw_bbox(std::vector<cv::Mat>& img_batch, std::vector<std::vector<Detection>>& res_batch) {
  for (size_t i = 0; i < img_batch.size(); i++) {
    ...
    for (size_t j = 0; j < res.size(); j++) {
      ...
      // cv::putText(img, std::to_string((int)res[j].class_id), cv::Point(r.x, r.y - 1), cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(0xFF, 0xFF, 0xFF), 2);
      cv::putText(img, name +": "+ std::to_string(confidence), cv::Point(r.x, r.y - 1), cv::FONT_HERSHEY_PLAIN, 2.5, cv::Scalar(0, 0x00, 255), 2);
    }
  }
}
